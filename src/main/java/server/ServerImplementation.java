 ...
 * e rappresenta l'oggetto remoto che sarà accessibile dai client tramite RMI.
...
public class ServerImplementation  extends UnicastRemoteObject implements EmotSongsInterface{
     ...
     * Se l'utente non è loggato, viene creata la prima sessione per l'utente e viene aggiunta alla mappa delle sessioni utente,
    ...
    public synchronized UUID loginSession(String username) throws RemoteException {
        ...
        if (userSessions != null && !userSessions.isEmpty()) {
            ...
            System.out.println(username + " già loggato - Nuova SessioneID: " + sessionId);
    ...
    /**
     * Verifica la disponibilità di un determinato userID.
     * Esegue una query per contare il numero di occorrenze del userID specificato.
     * @param user L'userID da verificare.
     * @return True se l'userID è disponibile (non presente nel database), altrimenti false.
     * @throws RemoteException Se si verifica un errore durante l'esecuzione della chiamata remota.
    ...
    public synchronized boolean isUserIDAvailable(String user) throws RemoteException, SQLException {
        ...
        try {
            ...
            throw new SQLException("Errore durante la query per verificare la disponibilità dell'userID: " + e.getMessage());
        } finally {
            if (statement != null) {
                statement.close();
            }
     ...
     * @throws RemoteException Se si verifica un errore durante l'esecuzione della chiamata remota.
     * @throws UtenteInesistenteException Se l'utente non è presente nel database.
            ...
            for (Emozione emozione : datiEmotion) {
                ...
                if (result.next()) {
                    ...
                    if (existingScore != newScore || (existingNotes == null || existingNotes.isEmpty()) || !existingNotes.equals(newNotes)) {
                        ...
                        System.out.println("Il record esiste già con lo stesso score, non è necessario aggiornare.");
                    }
                } else {
